...
%w[en ca es].each do |lang|
  ...
  breweries.each do |brewery|
    ...
    brewer = { name: company, location: location, tap: tap, uuid: SecureRandom.uuid }
    firebase.push('breweries', brewer) if lang == 'en'
    ...
    beers.each do |beer|
      ...
      beer = (all_beers[number] ||= { brewery: brewer })
      beer.merge!(id: number, name: name, "description_#{lang}": description, tags: tags,
...
# raise all_beers.inspect
all_beers.each do |_, beer|
  puts beer
  firebase.push('beers', beer)
end
